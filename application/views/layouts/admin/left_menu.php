...
<nav>
	...
	<?php
$menu_open = false;
if ($this->router->fetch_class() == "template" || $this->router->fetch_class() == "group" || $this->router->fetch_class() == "group_leads" || $this->router->fetch_class() == "leads" || $this->router->fetch_class() == "smtp") {
    $menu_open = true;
}
?>
        <li
			class="treeview <?php if($menu_open==true){?>menu-open<?php }?>"><a
			href="javascript:void(0)"><i class="icon_document_alt"></i> <span>Email
					Management</span> <i class="fa fa-angle-right"></i></a>
			<ul class="treeview-menu" <?php if($menu_open==true){?>
				style="display: block;" <?php }?>>
				<li <?php if($this->router->fetch_class()=="smtp"){?> class="active"
					<?php }?>><a href="<?php echo site_url('admin/smtp/index'); ?>">-Smtp</a></li>
				<li <?php if($this->router->fetch_class()=="leads"){?>
					class="active" <?php }?>><a
					href="<?php echo site_url('admin/leads/index'); ?>">-Leads</a></li>
				<li <?php if($this->router->fetch_class()=="group"){?>
					class="active" <?php }?>><a
					href="<?php echo site_url('admin/group/index'); ?>">-Group</a></li>
				<li <?php if($this->router->fetch_class()=="group_leads"){?>
					class="active" <?php }?>><a
					href="<?php echo site_url('admin/group_leads/index'); ?>">-Group
						Leads</a></li>
				<li <?php if($this->router->fetch_class()=="template"){?>
					class="active" <?php }?>><a
					href="<?php echo site_url('admin/template/index'); ?>">-Template</a></li>
			</ul></li>
